  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
      ...
      <div className="flex-1 min-w-0">
        ...
        {file.error ? (
          <p className="text-[10px] text-rose-500 font-medium mt-0.5">{file.error}</p>
        ) : (
      ...
      {done ? (
        ...
        <button
          ...
          onClick={onRemove}
          title={file.error}
          className="text-rose-400 hover:text-rose-600 transition-colors flex-shrink-0 p-1 rounded-lg hover:bg-rose-50"
        >
          <TrashIcon />
        </button>
      ) : (
...
export default function SubmitModal({
  ...
  const uploadFile = useCallback((item) => {
    ...
    const path     = `submissions/week${selectedWeek}/${currentUser.uid}/${Date.now()}_${safeName}`;
    const taskRef  = ref(storage, path);
    const task     = uploadBytesResumable(taskRef, item.file);
    ...
    task.on(
      ...
      (err) => {
        ...
        setPendingFiles((prev) =>
          prev.map((f) => (f.id === item.id ? { ...f, error: msg, progress: 0 } : f))
        );
      },
      async () => {
        const url = await getDownloadURL(task.snapshot.ref);
        setPendingFiles((prev) =>
          prev.map((f) => (f.id === item.id ? { ...f, progress: 100, url, error: null } : f))
        );
      }
    );
  ...
  const addFiles = useCallback((fileList) => {
    ...
    const newFiles = Array.from(fileList)
      .filter((f) => {
        if (!ALL_ACCEPT.includes(f.type) && !f.type.startsWith('image/') && !f.type.startsWith('video/')) {
          rejected.push(f.name + ' (unsupported type)');
          return false;
        }
        if (f.size > 100 * 1024 * 1024) { // 100 MB max
          rejected.push(f.name + ' (over 100 MB)');
          return false;
        }
        return true;
      })
    ...
    if (rejected.length > 0) {
      setErrors((p) => ({ ...p, general: `Skipped: ${rejected.join(', ')}` }));
    }
    if (newFiles.length === 0) return;

    // Add to state first, then start uploads separately (avoid side effects in setState)
